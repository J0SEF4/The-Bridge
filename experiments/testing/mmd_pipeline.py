...
# "the smaller the distance of the pair is, the larger its weight should be"
# This transfers weight from further pairs to closer ones via NN's NN correspondences
...
def compute_mmd_detailed(D: np.ndarray,
    ...
    # Part 1: For each C_i in M1, find NN's NN correspondence
    for i in range(m):
        # C'_N(i): Find NN of C_i in M2
        j_N_i = int(np.argmin(D[i, :]))
        # C_N'(N(i)): Find NN of C'_N(i) back in M1 (the NN's NN)
        ...
        matches.append({
            ...
            'd_j': dj,
            'r_ij': r,
            'id_i': idsA[i_N_prime_N_i] if idsA is not None else None,
            'id_j': idsB[j_N_i] if idsB is not None else None,
            'label_i': labelsA[i_N_prime_N_i] if labelsA is not None else None,
            'label_j': labelsB[j_N_i] if labelsB is not None else None,
        })
    
    # Part 2: For each C'_j in M2, find NN's NN correspondence
    for j in range(n):
        # C_N'(j): Find NN of C'_j in M1
        i_N_prime_j = int(np.argmin(D[:, j]))
        # C'_N(N'(j)): Find NN of C_N'(j) back in M2 (the NN's NN)
    ...
    # Deduplicate matches (same pairs from both perspectives)
    uniq = []
    seen = set()
    for mm in matches:
        key = (mm['i'], mm['j'])
    ...
    # Compute final MMD using consistent mean distance (Eq. 21)
    unique_distances = [mm['d_ij'] for mm in uniq]
    d_mmd = float(np.mean(unique_distances)) if unique_distances else 0.0
    ...
    info = {
        ...
        'patchesA_count': m,
        'patchesB_count': n,
        'mean_pair_dim': float(np.mean([mm['r_ij'] for mm in uniq])) if uniq else 0.0,
        'median_pair_dim': float(np.median([mm['r_ij'] for mm in uniq])) if uniq else 0.0,
    }
    return d_mmd, info